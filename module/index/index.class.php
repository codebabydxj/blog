<?php
	class index extends indexMain{
		function init(){
			...
			//点击量
			$liulan=$this->db->select("select * from shows where status=3 order by hits desc limit 0,10");
			$this->smarty->assign("liulan",$liulan);
			//点赞
            $dianzan=$this->db->select("select * from shows where status=3 order by good desc limit 0,10");
		...
		function code(){
			$code=new code();
			$code->fontFamily="static/font/FZBSFW.TTF";
			$code->output();
			$session=new session();
		...
		function check(){
	        ...
	        $uname=P("uname");
	        if($uname=="admin"){
	        	$this->jump("index.php?m=index&a=login","登录失败");
	        	exit;
	        }
	        $upass=md5(P("upass"));
	        $db=new db("user");
		...
		function logout(){
	      	...
	      	$this->jump("index.php?m=index","退出登录成功");
	    }
	}
?>
